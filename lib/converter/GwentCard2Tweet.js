...
const INVERTED_POWER_CARDS = require("../GwentInvertedPowerCards"); // 戦力増減判定が逆になるカードのリスト
...
class GwentCards2Tweet {
  ...
  saveTweetJP(dpath_v = '', lang = 'ja-jp') {
    ...
    // 勢力ごとのバフ・ナーフ統計を集計する
    // INVERTED_POWER_CARDS に含まれるカード（密偵型など）は戦力増減の判定が逆になる
    const countPower = (faction, diffType, forInverted) => {
      const cards = grouped.card[faction]?.[Diff[diffType]] ?? [];
      return cards.filter(card => INVERTED_POWER_CARDS.has(card.CardId) === forInverted).length;
    };
    ...
    factionOrder.forEach(faction => {
      factionStats[faction] = {
        buff: (grouped.leader[faction]?.[Diff["ProvisionBuff"]]?.length ?? 0)
            ...
            + countPower(faction, "PowerBuff", false)
            + countPower(faction, "PowerNerf", true),
        nerf: (grouped.leader[faction]?.[Diff["ProvisionNerf"]]?.length ?? 0)
            + (grouped.card[faction]?.[Diff["ProvisionNerf"]]?.length ?? 0)
            + countPower(faction, "PowerNerf", false)
            + countPower(faction, "PowerBuff", true)
      };
    });
