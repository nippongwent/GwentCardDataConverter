...
class GwentCards2CSV {
  ...
  saveFile(lang = 'ja-jp') {
    ...
    // 表頭の設定
    // Keywords と DebugName は内部的なデータなのでCSVに展開していない
    let Header = Object.keys(Cards[0]);
    output_text += '"' + Header.join('","').replace(/"DebugName",/, "").replace(/"Keywords",/, "") + '"' + ',"URL"' + NL;
    ...
    Cards.forEach(Card => {
      ...
      if(lang == 'ja-jp') {
        output_text += `"https://www.playgwent.com/ja/decks/builder/card/details/${Card.CardId}"`;
      } else {
        output_text += `"https://www.playgwent.com/en/decks/builder/card/details/${Card.CardId}"`;
      }
      output_text += NL;
    });
