...
class GwentCards2Seesaa {
  ...
  make(lang = 'ja-jp') {
    ...
    let dpath_v_l = path.join(
      dpath_v,
      "listing"
    );
    if(!fs.existsSync(dpath_v_l)) fs.mkdirSync(dpath_v_l);
    ...
    this.saveKeyword(dpath_v_l, lang);
    this.saveCategory(dpath_v_l, lang);
  }
  ...
  saveFaction(dpath_v_f = '', lang = 'ja-jp') {
    ...
    for(let faction in Factions2Save) {
      let output_text  = `// [GCDC:StartFaction:${faction}]` + `${NL}`;
      ...
      output_text += `|}` + `${NL}`;
      output_text += `// [GCDC:EndFaction:${faction}]` + `${NL}${NL}${NL}`;
  ...
  saveCardset(dpath_v_c = '', lang = 'ja-jp') {
    ...
    for(let cardset in Cardset2Save) {
      let output_text  = `// [GCDC:StartCardset:${cardset}]` + `${NL}`;
      output_text     += cautionary_statement;
      output_text     += `// ${this.GwentCards.CardData.Version}` + `${NL}`;
      ...
      output_text += `// [GCDC:EndCardset:${cardset}]` + `${NL}${NL}${NL}`;
  ...
  lang  = 'ja-jp'
  単体で使わず make() で全部作ることを推奨
  保存のファイル名は keyword.utf8.txt に固定してある
  --------------------------------------------------------*/
  saveKeyword(dpath_v_l = '', lang = 'ja-jp') {
    let Cards       = this.GwentCards.CardData[lang].Cards;

    // 全キーワードの配列を作成する
    let Keyword2Save = Object.keys(this.GwentCards.CardData[lang].Keywords);
    ...
    // それぞれのカードセットを保存していく
    let output_text = "";
    Keyword2Save.forEach( keyword => {
      // keyword の訳を詳細から持ってくる
      let keyword_loc = this.GwentCards.CardData[lang].Keywords[keyword].replace(':', '：').split('：')[0];

      output_text += `// [GCDC:StartKeyword:${keyword_loc}]` + `${NL}`;
      output_text += cautionary_statement;
      output_text += `// ${this.GwentCards.CardData.Version}` + `${NL}`;
      ...
      FACTION_ORDER.forEach( faction => {
        ...
        Cards.forEach(Card => {
          ...
          // 必要ないものを弾く
          if(!Card.Keywords.includes(keyword)) return;
          if(Card.Faction !== faction) return;
          if(Card.CardType === "リーダーアビリティ") return;
          if(Card.Cardset  === "トークン" && Card.CardType !== "ストラタジェム") return;
          ...
          // どのカードのデータを作成するかを決めたら
          // this.seesaaTableFormat() で個々のカードを変換する
          faction_text  += this.seesaaListFormat(Card,1);
          faction_count += 1;
        });

        if(faction_count === 0) {
          output_text += `** ${faction}` + `${NL}`;
          output_text += `&color(#686868){（該当カードは存在しない）}` + `${NL}${NL}`;
        } else {
          output_text += faction_text + `${NL}`;
        }
      });
      output_text += `// [GCDC:EndKeyword:${keyword_loc}]` + `${NL}${NL}${NL}`;
    });
  ...
  lang  = 'ja-jp'
  単体で使わず make() で全部作ることを推奨
  保存のファイル名は category.utf8.txt に固定してある
  --------------------------------------------------------*/
  saveCategory(dpath_v_l = '', lang = 'ja-jp') {
    let Cards       = this.GwentCards.CardData[lang].Cards;

    // 全キーワードの配列を作成する
    let Category2Save = this.GwentCards.CardData[lang].Categories;
    ...
    // それぞれのカードセットを保存していく
    let output_text = "";
    Category2Save.forEach( category => {
      if(category == null) return;

      output_text += `// [GCDC:StartCategory:${category}]` + `${NL}`;
      output_text += cautionary_statement;
      output_text += `// ${this.GwentCards.CardData.Version}` + `${NL}`;
      ...
      FACTION_ORDER.forEach( faction => {
        ...
        Cards.forEach(Card => {
          ...
          // 必要ないものを弾く
          if(!Card.Categories.includes(category)) return;
          if(Card.Faction !== faction) return;
          if(Card.CardType === "リーダーアビリティ") return;
          if(Card.Cardset  === "トークン" && Card.CardType !== "ストラタジェム") return;
    
          // どのカードのデータを作成するかを決めたら
          // this.seesaaListFormat() で個々のカードを変換する
          faction_text  += this.seesaaListFormat(Card,1);
          faction_count += 1;
        });
      ...
      output_text += `// [GCDC:EndCategory:${category}]` + `${NL}${NL}${NL}`;
    });
  ...
  seesaaCardFormat(Card, lang = 'ja-jp') {
    ...
    // カードデータのヘッダー
    seesaa_text  = `// [GCDC:StartCard:${Card.CardId}] - Version: ${this.GwentCards.CardData.Version}` + `${NL}`;
    seesaa_text += `|w(100):center:bgcolor(#333333):color(#f3f3fe)|left:|c` + `${NL}`;

    // カードデータの本体
    seesaa_text += `|!カード名|${Card.Name}|` + `${NL}`;
    seesaa_text += `|!英語カード名|${Card.NameGlobal}|` + `${NL}`;
    ...
    seesaa_text += `|!更新|${this.GwentCards.CardData.LastUpdate}|` + `${NL}`;
    ...
    seesaa_text += `// [GCDC:EndCard:${Card.CardId}]` + `${NL}${NL}${NL}`;
  ...
  特定のカード一覧のテキストを返す関数
  --------------------------------------------------------*/
  seesaaListFormat(Card, option = 0) {
    let seesaa_text = "";
