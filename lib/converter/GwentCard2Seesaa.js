...
class GwentCards2Seesaa {
  ...
  seesaaCardFormat(Card, lang = 'ja-jp') {
    ...
    if(Card.Changelog !== undefined) {
      seesaa_text += `// ------------------------------` + `${NL}`;;
      seesaa_text += `// [Changelog]: ${Card.Changelog.Summary}` + `${NL}`;
      for(let prop in Card.Changelog) {
        if(prop === "Summary") continue;
        if(prop === "Ability" || prop === "Flavor") {
          seesaa_text += `// [${prop} - Previous]:` + `${NL}`;
          seesaa_text += `// ${String(Card.Changelog[prop]).replace(/\\n/g, `${NL}// `)}` + `${NL}`;
          continue;
        }

        // 通常のプロパティ
        seesaa_text += `// [${prop}]: ${Card.Changelog[prop]} -> ${Card[prop]}` + `${NL}`;
      }
      seesaa_text += `// [PreviousVersion]: ${this.GwentCards.CardData.PreviousVersion}` + `${NL}`;
      seesaa_text += `// ------------------------------` + `${NL}`;
    }
